...
import { FaMobileAlt, FaCompass, FaCheckCircle, FaExclamationTriangle, FaLock } from 'react-icons/fa';
    ...
    <div className="min-h-screen bg-dark-bg flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md glass p-8 md:p-10 rounded-3xl shadow-2xl border border-white/5 relative z-10 animate-fade-in">
        ...
        <div className="text-center space-y-4 mb-8">
          <div className={`inline-flex p-4 rounded-2xl ${success ? 'bg-emerald-800/85' : 'bg-forest-800/85'} text-white shadow-xl`}>
        ...
        {success ? (
          ...
          <form onSubmit={otpSent ? handleVerifyOTP : handleRequestOTP} className="space-y-6">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Mobile Number</label>
              <div className="relative">
                ...
                <input
                  ...
                  disabled={otpSent}
                  className="w-full bg-white/5 border border-white/10 rounded-2xl py-3.5 pl-11 pr-4 text-white placeholder-gray-500 focus:outline-none focus:border-forest-500 focus:bg-white/10 transition-all text-sm disabled:opacity-50"
                  required
                />
              </div>
            </div>
            ...
            {otpSent && (
              <div className="space-y-2 animate-fade-in">
                <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider block">Enter 4-Digit OTP</label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-400">
            ...
            {error && (
              <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-3.5 text-xs text-red-400 flex items-start gap-2.5 animate-fade-in leading-relaxed">
                <FaExclamationTriangle className="w-4.5 h-4.5 shrink-0 text-red-500 mt-0.5" />
                <span>{error}</span>
              </div>
            )}
